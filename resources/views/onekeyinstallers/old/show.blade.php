@extends('adminlte::page')

@section('title', __('onekeyinstallers.title'))

@section('content_header')
    <h1 class="m-0 text-dark">{{ __('onekeyinstallers.header') }}</h1>
@stop

@section('content')
    <div class="row">
        <div class="col-lg-12 margin-tb">
            <div class="pull-left">
                <h1>{{ __('tables.details') }}</h1>
            </div>
            @include('layouts.back')
        </div>
    </div>

    <div class="row">
         <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>{{ __('onekeyinstallers.id') }} :</strong>
                {{ $onekeyinstaller->id }}
            </div>
         </div>
         <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>{{ __('onekeyinstallers.label') }} :</strong>
                {{ $onekeyinstaller->apk ? $onekeyinstaller->apk->label : $onekeyinstaller->label }}
            </div>
         </div>
         <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>{{ __('onekeyinstallers.project') }} :</strong>
                {{ $onekeyinstaller->project ? $onekeyinstaller->project->name : __('onekeyinstallers.project_all') }}
            </div>
         </div>
         <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>{{ __('onekeyinstallers.package_name') }} :</strong>
                {{ $onekeyinstaller->package_name }}
            </div>
         </div>
         <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>{{ __('onekeyinstallers.url') }} :</strong>
                {{ $onekeyinstaller->url }}
            </div>
         </div>
         <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>{{ __('tables.creator') }} : </strong>
                {{ $onekeyinstaller->user->name }}
            </div>
         </div>
         <div class="col-xs-12 col-sm-12 col-md-12">
            <div class="form-group">
                <strong>{{ __('onekeyinstallers.status') }} :</strong>
                {{ ($onekeyinstaller->status==1) ? __('tables.status_on'):__('tables.status_off') }}
            </div>
         </div>
     </div>
@endsection
